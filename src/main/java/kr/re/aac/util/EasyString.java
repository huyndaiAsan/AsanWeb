...
public class EasyString {
	...
	*/
	...
	public static String xssReplace(String oriStr ){
	    ...
	    if ( cntInStr(replaceStr.toLowerCase() , "onmouseover" ) > 0 ) {   replaceStr = replaceStr.toLowerCase().replaceAll("onmouseover", ""); }
	    if ( cntInStr(replaceStr.toLowerCase() , "alert" ) > 0 ) {   replaceStr = replaceStr.toLowerCase().replaceAll("alert", ""); }
		...
		try {
			if (!filstr.equals("")) { 
				...
				for( int x = 0; x < st.length; x++ ) { 
					str = str.replaceAll(st[x].replaceAll(" ",""), "_"+st[x]+"_"); 
				} 
			} 
		} catch (Exception e) {
			e.printStackTrace();
